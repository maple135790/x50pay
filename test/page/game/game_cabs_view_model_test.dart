...
void main() {
  ...
  void arrangeGetGameListReturnsGameList() {
    when(
      () => mockRepo.getGameList(storeId: any(named: 'storeId')),
    ).thenAnswer((_) async {
      const rawResponse =
  ...
  void arrangeGetGameListReturnsEmptyGameList() {
    when(() => mockRepo.getGameList(storeId: any(named: 'storeId'))).thenAnswer(
      (_) async {
  ...
  group('切換Tab', () {
    ...
    test('當切換tab時，點選為店家，取得店家資料', () async {
      ...
      arrangeRememberGameTabAndTabToStoreGame();
      final viewModel =
          GameCabsViewModel(repository: mockRepo, currentLocale: fakeLocale)
