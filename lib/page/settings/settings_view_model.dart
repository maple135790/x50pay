...
class SettingsViewModel extends BaseViewModel {
  ...
  /// 取得獲券紀錄
  Future<TicDateLogModel> getTicketLog({
    int debugFlag = 200,
  }) async {
    await EasyLoading.show();
    await Future.delayed(const Duration(milliseconds: 200));
  ...
  /// 取得P點付費明細
  Future<PlayRecordModel> getPlayRecord({
    int debugFlag = 200,
  }) async {
    await EasyLoading.show();
    await Future.delayed(const Duration(milliseconds: 200));
  ...
  /// 取得回饋點數明細
  Future<FreePointModel> getFreePointRecord({
    int debugFlag = 200,
  }) async {
    await EasyLoading.show();
    await Future.delayed(const Duration(milliseconds: 200));
    ...
    try {
      if (!kDebugMode || isForceFetch) {
        ...
        if (debugFlag != 200) {
          freePModel = FreePointModel.fromJson(
              jsonDecode(testFreePointRecord(code: 700)));
        } else {
          freePModel =
              FreePointModel.fromJson(jsonDecode(testFreePointRecord()));
        }
      }
  ...
  String testFreePointRecord({int code = 200}) =>
      r'''{"message":"done","code":$code,"log":[{"_id":{"$oid":"64f9f38253a65c8baa2855fa"},"uid":"X938","limittime":"2023-10-08 00:00","fpoint":25.0,"much":25.0,"expire":false}]}''';
